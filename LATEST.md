...
## Recently tried
- 2026-09-08: Renamed the product label to "LCA Model" (folder/code stay `lcagraph`); "Try our Process Calculators" is now a plain dropdown with four items (Case Studies, LCA Step by Step, LCA Model, LCA Graph (Beta)); the green pill styling on that menu entry is gone; the Resources page tab strip is removed and the section shown follows the URL hash.
